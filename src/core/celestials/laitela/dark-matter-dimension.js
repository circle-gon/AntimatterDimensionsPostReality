...
export class DarkMatterDimensionState extends DimensionState {
  ...
  get commonDarkMult() {
    return DC.D1.timesEffectsOf(
      ...
      AtomUpgrade(2).effects.darkFromAu2
    );
  }
  ...
  get powerDM() {
    ...
    return Decimal.pow(1.15, this.data.powerDMUpgrades)
      .mul(2)
      .add(1)
  ...
  get powerDE() {
    ...
    return Decimal.pow(1.005, this.data.powerDEUpgrades)
  ...
  tick(realDiff) {
    ...
    for (let tier = 4; tier >= 1; tier--) {
      ...
      if (dim.interval < dim.timeSinceLastUpdate) {
        ...
        Currency.darkEnergy.add(dim.powerDE.mul(ticks));
