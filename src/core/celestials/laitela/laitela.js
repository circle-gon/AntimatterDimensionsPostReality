...
export const Laitela = {
  ...
  get matterExtraPurchaseFactor() {
    ...
    const postAtom = preAtom * AtomicParticle(0).effects[0] * Effects.product(AtomUpgrade(4));
    return postAtom;
  },
  get realityReward() {
  ...
  get darkMatterMultRatio() {
    return this.darkMatterMultGain.div(this.celestial.darkMatterMult).add(1);
  },
  ...
  maxAllDMDimensions(maxTier) {
    ...
    while (upgradeInfo.some((upgrade) => upgrade[0].lte(darkMatter) && upgrade[2] > 0 && upgrade[3] + 1 > upgrade[3])) {
