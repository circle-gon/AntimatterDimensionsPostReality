...
export const GlyphGenerator = {
  ...
  randomStrength(rng) {
    ...
    if (Ra.unlocks.maxGlyphRarityAndShardSacrificeBoost.canBeApplied || AtomMilestone.am6.isReached) return rarityToStrength(100);
  ...
  randomNumberOfEffects(type, strength, level, rng) {
    ...
    if (type !== "effarig" && (AtomMilestone.am7.isReached || Ra.unlocks.glyphEffectCount.canBeApplied)) return 4;
    if (type === "effarig" && AtomMilestone.am7.isReached) return 7;
