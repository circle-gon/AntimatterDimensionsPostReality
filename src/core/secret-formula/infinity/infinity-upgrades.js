...
export const infinityUpgrades = {
  totalTimeMult: {
    id: "timeMult",
    cost: 1,
    description: () => `Antimatter Dimensions gain a multiplier based on time played${atomTimeText()}`,
    effect: () => Time.thisCollapse.totalMinutes.div(2).pow(0.15),
    formatEffect: (value) => formatX(value, 2, 2),
    charged: {
      description: () => `Antimatter Dimensions gain a power effect based on time played${atomTimeText()} and Teresa level`,
      effect: () =>
        1 + (Math.log10(Time.thisCollapse.totalMilliseconds.log10()) * Math.pow(Ra.pets.teresa.level, 0.5)) / 150,
  ...
  buy10Mult: {
    ...
    charged: {
      description: () =>
        `The multiplier for buying ${formatInt(10)} Antimatter Dimensions gains ` +
  ...
  thisInfinityTimeMult: {
    ...
    effect: () => Time.thisInfinity.totalMinutes.div(4).pow(0.25).max(1),
    formatEffect: (value) => formatX(value, 2, 2),
    charged: {
      description: "Antimatter Dimensions gain a power effect based on time spent in current Infinity and Teresa level",
      effect: () =>
        1 + (Math.log10(Time.thisInfinity.totalMilliseconds.add(100).log10()) * Math.sqrt(Ra.pets.teresa.level)) / 150,
      formatEffect: (value) => formatPow(value, 4, 4),
    },
  },
  unspentIPMult: {
    ...
    charged: {
      ...
      effect: () =>
        Currency.infinityPoints.value
          .dividedBy(2)
          .pow(Math.sqrt(Ra.pets.teresa.level) * 1.5)
          .plus(1),
      formatEffect: (value) => formatX(value, 2, 2),
    },
  },
  ...
  ipGen: {
    ...
    formatEffect: (value) => {
      ...
      if (player.records.bestInfinity.time.gte(Decimal.MAX_LIMIT)) return "Too slow to generate";
    ...
    charged: {
      ...
      effect: () =>
        Math.pow(Ra.pets.teresa.level, 2) * Ra.unlocks.continuousTTBoost.effects.autoPrestige.effectOrDefault(1),
