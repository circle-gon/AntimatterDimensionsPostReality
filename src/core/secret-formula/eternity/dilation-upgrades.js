...
function rebuyable(config) {
  const isPelleOnly = Boolean(config.pelleOnly);
  return {
    id: config.id,
    cost: () =>
      rebuyableCost(config.initialCost, config.increment, config.id, isPelleOnly ? AtomUpgrades.dilExpo(config.id) : 1),
