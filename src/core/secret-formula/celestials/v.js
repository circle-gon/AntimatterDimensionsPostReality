...
export const v = {
  ...
  runUnlocks: [
    ...
    },
    {
      ...
      description: (value) =>
        `Unlock Reality with at most ${formatInt(-value)} Glyphs equipped for the entire Reality.`,
      // This achievement has internally negated values since the check is always greater than
      values: [1, 4, 7, 10, 13],
      condition: () => V.isRunning && TimeStudy.reality.isBought,
    ...
    fastAutoEC: {
      ...
      format: (x) =>
        Ra.unlocks.instantECAndRealityUpgradeAutobuyers.canBeApplied
          ? "Instant (Ra upgrade)"
          : `${DecimalTimeSpan.fromMinutes(Decimal.recip(x).mul(1200)).toStringShort()} for full completion`,
