...
export const general = {
  achievement: {
    name: (ach, dim) => (dim?.length === 2 ? `Achievement ${ach} (${dim})` : `Achievement ${ach}`),
    multValue: (ach, dim) => {
      // There is also a banked infinities gain effect, but we don't track that in the multiplier tab
      if (ach === 131)
        return Achievement(131).canBeApplied ? Achievement(131).effects.infinitiesGain.effectOrDefault(1) : 1;
      ...
      if (dim?.length === 2) {
        ...
        for (let tier = 1; tier <= MultiplierTabHelper.activeDimCount(dim); tier++) {
          ...
          if (ach === 43) singleEffect = Achievement(43).canBeApplied ? 1 + tier / 100 : 1;
            ...
            singleEffect =
              MultiplierTabHelper.achievementDimCheck(ach, `${dim}${tier}`) && Achievement(ach).canBeApplied
  ...
  timeStudy: {
    name: (ts, dim) => (dim?.length === 2 ? `Time Study ${ts} (${dim})` : `Time Study ${ts}`),
    multValue: (ts, dim) => {
