...
export const TD = {
  total: {
    ...
    displayOverride: (dim) =>
      dim ? formatX(TimeDimension(dim).multiplier, 2) : `${format(TimeDimension(1).productionPerSecond, 2)}/sec`,
    multValue: (dim) =>
      dim
  ...
  highestDim: {
    ...
    displayOverride: () => {
      ...
      return `TD ${dim}, ${formatInt(TimeDimension(dim).totalAmount)}`;
    },
    multValue: () => TimeDimension(MultiplierTabHelper.activeDimCount("TD")).totalAmount,
  ...
  basePurchase: {
    ...
    multValue: (dim) => {
      const getMult = (td) =>
        Decimal.pow(4, td === 8 ? Math.clampMax(TimeDimension(td).bought, 1e8) : TimeDimension(td).bought);
      if (dim) return getMult(dim);
      return TimeDimensions.all
        .filter((td) => td.isProducing)
        .map((td) => getMult(td.tier))
  ...
  achievementMult: {
    ...
    multValue: (dim) =>
      Decimal.pow(EternityUpgrade.tdMultAchs.effectOrDefault(1), dim ? 1 : MultiplierTabHelper.activeDimCount("TD")),
  ...
  dilationUpgrade: {
    ...
    multValue: (dim) => {
      const mult =
        Replicanti.areUnlocked && Replicanti.amount.gt(1) ? DilationUpgrade.tdMultReplicanti.effectValue : DC.D1;
  ...
  realityUpgrade: {
    ...
    multValue: (dim) =>
      Decimal.pow(RealityUpgrade(22).effectOrDefault(1), dim ? 1 : MultiplierTabHelper.activeDimCount("TD")),
  ...
  pelle: {
    ...
    multValue: (dim) =>
      Decimal.pow(PelleRifts.chaos.effectOrDefault(1), dim ? 1 : MultiplierTabHelper.activeDimCount("TD")),
  ...
  iap: {
    ...
    multValue: (dim) =>
      Decimal.pow(ShopPurchase.allDimPurchases.currentMult, dim ? 1 : MultiplierTabHelper.activeDimCount("TD")),
