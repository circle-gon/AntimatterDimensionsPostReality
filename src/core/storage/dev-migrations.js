  ...
  patches: [
    ...
    player => {
      ...
      movePropIfPossible("teresa", "effarig", "autoGlyphSac", {
        ...
        types: GlyphTypes.list.mapToObject(
          t => t.id,
          t => ({
            rarityThreshold: 0,
            scoreThreshold: 0,
    ...
    player => {
      player.bestGlyphStrength = player.reality.glyphs.active
    ...
    player => {
      for (const key of Object.keys(player.reality.automator.scripts)) {
        ...
        for (let num = 0; num < lines.length; num++) {
          ...
          // TT command removed
          rawLine = rawLine.replace(/^\s*tt.*$/iu, "");
          ...
          rawLine = rawLine.replace(/x current/iu, "x highest");
          // Variable definitions
          const defineMatch = rawLine.match(/define (\S*)\s*=\s*(\S.*)$/iu);
          if (defineMatch) {
