...
CodeMirror.defineSimpleMode("automato", {
  ...
  glyphArgs: [
    commentRule,
    { sol: true, next: "start" },
    { regex: /equip/ui, token: "variable-2", next: "equipGlyphArgs" },
  ],
  equipGlyphArgs: [
    commentRule,
    { sol: true, next: "start" },
    { regex: /(pow(er)?|inf(inity)?|rep(lication)?|time|dil(ation)?|eff(arig)?|reality)/ui, token: "variable", next: "commandDone" },
  ],
  prestige: [
    commentRule,
    { sol: true, next: "start" },
