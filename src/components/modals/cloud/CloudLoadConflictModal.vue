...
<template>
  <ModalWrapperChoice
    ...
    :confirm-fn="confirm"
  >
    <template #header> Load Game from Cloud </template>
    <span v-if="hasDifferentName"> Your Local and Cloud Saves have <b>different names</b>. </span>
    <span v-else-if="older"> Loading from the Cloud would <b>load a save with less playtime</b>. </span>
    <span v-else-if="farther"> Loading from the Cloud would <b>cause you to lose progress</b>. </span>
    ...
    <template #cancel-text> Keep Local Save </template>
    <template #confirm-text> Overwrite Local with Cloud Save </template>
  </ModalWrapperChoice>
</template>
